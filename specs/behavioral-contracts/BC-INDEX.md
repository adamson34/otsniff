...
- BC-3.06.006 MITRE ATT&CK for ICS technique coverage: every **adversary-detection** rule in `catalog()` declares ≥1 `ReferenceKind::MitreIcsAttack` reference whose `url` is `Some` and matches `https://attack.mitre.org/techniques/T0\d+/`; the 7 previously-unmapped rules (dns_resolver→T0884, ldap_creds→T0859, ntp_external→T0884, plaintext_creds→T0859, smbv1→T0866, stale_tls→T0830, weak_tls_cipher→T0830) gain mappings; supporting (not fire-equals-technique) mappings carry a `(supporting)` suffix in the label. **Exempt:** the 3 policy-gated `zonewarden.*` conformance verdicts (idmz_bypass / wrong_direction / deny_by_default) — they are IEC 62443 *segmentation conformance* checks, not adversary techniques; ATT&CK for ICS models adversary techniques (network segmentation is a mitigation, M0930, not a technique), so forcing a technique would be semantically wrong. They keep their IEC 62443 `Spec` references. The coverage test skips ids prefixed `zonewarden.`. MITRE data lives ONLY in the catalog — the single source of truth (ADR-0014), never duplicated onto `Finding` (HIGH, added S-12.01 v0.6.0)
...
- BC-3.08.005 Diff capture-window-mismatch surfacing: when a side's window is degenerate (`rate_normalized == false`) or the two usable windows differ by `>= 2×` (inclusive, so the 1h-vs-30min case warns; matches the flow-shift detector's `>=` multiplier), `run_diff` emits a stderr `WARNING:` (via the shared `Diff::window_warning`, formatted through `fmt_window_secs` for stderr/banner numeric parity) and the diff HTML+MD reports render a conditional banner stating the window sizes and whether flow-shift ratios are rate-normalized or raw; windows differing by `< 2×` emit neither. Separately, whenever ratios are rate-normalized AND flow shifts exist, the flow-shift table heading reads "rate change" (not "volume change") and carries an explanatory rate-note — including the within-2× no-banner band — so a flagged flow with equal raw bytes isn't misread (HIGH, added S-11.01 v0.6.0)
