...
pub fn render_markdown(
    ...
    if findings.is_empty() {
        writeln!(
            out,
            "_No findings raised. Spot-check the asset inventory and top flows below — quiet captures sometimes mean the right things were filtered out at the SPAN port._"
        )
        .unwrap();
        writeln!(out).unwrap();
    } else {
    ...
    writeln!(
        out,
        "---\n\n_Generated by otsniff. Findings are heuristic — a clean report is not an audit. Validate with the on-site team before acting._"
    )
    .unwrap();
